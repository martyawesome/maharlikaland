      ...
      <div class="box-body">
        <ul class="row list-group">
          @foreach($gallery as $project_gallery)
            <li class="col-md-6 col-sm-6" style="list-style:none;" onclick="chooseImage(<?php echo $project_gallery->id?>)">
              <div class="box-body">
                <div class="box-body gallery-item">
                  <div class="selected-image-delete">
                    <img id="<?php echo $project_gallery->id?>" class="img-responsive gallery-image" src="<?php echo asset("").$project_gallery->image_path?>" alt="Gallery picture">
                  </div>
              </div>
            </li>
          @endforeach
    ...
    function chooseImage(imageId) {
      ...
      if(window.imageIds.length == 0) {
        ...
        $("#deletePhotosModalButton").click(function() {
          var url = "{{ URL::route('delete_project_images',array($project->slug,':ids')) }}";
