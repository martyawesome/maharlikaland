  ...
  <section class="content">
              ...
              <tbody>
                @foreach($water_bills as $water_bill)
                  <tr class='clickable-row' style="cursor: pointer;" data-href="{{ URL::route('view_bill_water_project',array($project->slug,$water_bill->date_covered)) }}">
                    ...
                    <td>{{ number_format($water_bill->details_consumption, 2, '.', ',') }}</td>
                    <td>{{ number_format($water_bill->details_bill, 2, '.', ',') }}</td>
                    <td>{{ number_format($water_bill->details_payment, 2, '.', ',') }}</td>
                  </tr>
                @endforeach
              </tbody>
            </table>
    ...
    <div style="margin-bottom:15px;">
      @if(Auth::user()->user_type_id == config('constants.USER_TYPE_DEVELOPER_ADMIN') 
        ...
        {!! link_to_route('add_bill_water_project', 'Add', array($project->slug), ['class' => 'btn btn-primary', 'style' => 'margin-right:5px;']) !!}
