<script>
   $(document).ready(function(){
    ...
    $("#submit-security-code-button").click(function(){
         $.ajax({
            type: "POST",
            url: "{{ url('/') }}/manage/developers/projects/{{ $project->slug }}/delete/joint_ventures/{{ $joint_venture->slug }}",
            data: {
              security_code : $('#security_code').val()
            },
            success: function (data) {
              if(data == 1){ 
                window.location="{{route('project_edit_joint_ventures',array($project->slug))}}";
              } else {
                if(data == 0){
