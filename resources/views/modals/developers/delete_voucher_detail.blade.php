<script>
   $(document).ready(function(){
    ...
    $("#submit-security-code-button").click(function(){
         $.ajax({
            type: "POST",
            url: "{{ url('/') }}/manage/developers/accounting/vouchers/{{ $project->slug }}/{{ $voucher->voucher_number }}/{{ $voucher_detail->id }}/delete",
            data: {
              security_code : $('#security_code').val()
            },
            success: function (data) {
              if(data == 1){ 
                window.location="{{route('voucher',array($project->slug, $voucher->voucher_number))}}";
              } else {
                if(data == 0){
