function chooseObject(objectId) {
  ...
  if(idIsFound) {
    $('#' + objectId).css('background-color','#ddd');
     window.objectIds = jQuery.grep(window.objectIds, function(value) {
      return value != objectId;
    });
  } else {
    $('#' + objectId).css('background-color','tomato');
  ...
  if(window.objectIds.length == 0) {
    ...
    $("#success-modal-button").click(function(){
       window.location="/manage/developers/marketing/promotional_materials/videos";
    });
    $("#submit-security-code-button").click(function(){
      var objectIds = btoa(JSON.stringify(window.objectIds));
      $.ajax({
          type: "POST",
          url: "/manage/developers/marketing/promotional_materials/videos/delete/"+objectIds,
          data: {
            security_code : $('#security_code').val()
          },
          success: function (data) {
            if(data == 1){ 
              $('#success-modal-message').html('The promotional videos were successfully deleted');
